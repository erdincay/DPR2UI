...
{
	m_bDrag = TRUE;
	m_pTheMainFrame = (CMainFrame*)AfxGetApp()->GetMainWnd();
}
...
//		return -1;*/
	m_image.Create(20, 20, ILC_COLOR24|ILC_MASK,2,2);
...
CComboBox* CMySuperGrid::ShowList(int nItem, int nCol, CStringList *lstItems)
{
	...
	DWORD dwStyle =  WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL|CBS_DROPDOWNLIST | CBS_DISABLENOSCROLL;
	...
	((CComboInListView*)pList)->m_strInitText = strFind;
	// The returned pointer should not be saved
	return pList;
}

void CMySuperGrid::ChangeListShow(int nItem, int nCol, CStringList *lstItems)
{
	CString strFind = GetItemText(nItem, nCol);
	...
	CString strDst;
	POSITION ps = lstItems->Find(strFind);
	if (ps != NULL)
	{
		lstItems->GetNext(ps);
		if (ps == NULL)
		{
			strDst = lstItems->GetHead();
		}
		else
		{
			strDst = lstItems->GetAt(ps);
		}
	}
	else
	{
		ps = lstItems->GetHeadPosition();
		if (ps == NULL)
		{
			strDst = lstItems->GetHead();
		}
	...
	SetItemText(nItem,nCol,strDst);
}

#define IDC_ZFTABLESELECTINLISTVIEW 0x1239

/*
CZFTableSelectList* CMySuperGrid::ShowZFSelect(int nItem, int nCol)
{
	CString strFind = GetItemText(nItem, nCol);
	...
	CZFTableSelectList *pZFTable = new CZFTableSelectList(nItem, nCol, strFind);
	pZFTable->Create(dwStyle, rect, this, IDC_COMBOBOXINLISTVIEW);
	pZFTable->ModifyStyleEx(0,WS_EX_CLIENTEDGE);//can we tell at all
...
void CMySuperGrid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
//	::PostMessage(GetParent()->GetSafeHwnd(),WM_REALDATA_LIST_DBCLICK,0,0);
	...
	{	
		...
		if(pInfo->GetControlType(ht.iSubItem-1, ctrlType))
		{	
			if(ctrlType == pInfo->CONTROLTYPE::datecontrol) 
			{
				CStringList* list=NULL;
				pInfo->GetListData(ht.iSubItem-1, list);
				ChangeListShow(ht.iItem, ht.iSubItem, list);
				CString val = GetItemText(ht.iItem, ht.iSubItem);
				pInfo->SetSubItemText(ht.iSubItem-1,val);

				return;
			}

			if (ctrlType == pInfo->CONTROLTYPE::spinbutton)
			{
				::PostMessage(GetParent()->GetSafeHwnd(),WM_PATCH_PROCESS,(WPARAM)pSelItem,0);
				return;
			}
	...
	CSuperGridCtrl::OnLButtonDblClk(nFlags, point);
}
