...
{
public:
	CItemInfo():m_bCheck(0),m_iImage((const int)-1),m_lParam(NULL),m_clf((COLORREF)-1),m_editEnable(FALSE)
	{
		setItemType(-1);
	}

	void SetItemText(const CString& strItem){ m_strItemName = strItem;m_editEnable = FALSE; }
	void SetItemText(const CString& strItem, COLORREF clf) { m_strItemName = strItem; m_clf = clf;m_editEnable = FALSE;}
	void SetItemText(const CString& strItem, COLORREF clf,BOOL editEnable) { m_strItemName = strItem; m_clf = clf;m_editEnable = editEnable;}
	void SetItemText(const CString& strItem, BOOL editEnable) { m_strItemName = strItem; m_editEnable = editEnable;}
	...
	void AddSubItemText(const CString& strSubItem,BOOL editEnable)
	{ 
		int nIndex = int(m_SubItems.Add(strSubItem)); 
		m_SubeditEnable.SetAtGrow(nIndex,editEnable);
	}
	void AddSubItemText(const CString& strSubItem, COLORREF clf ,BOOL editEnable)
	{ 
		int nIndex = int(m_SubItems.Add(strSubItem)); 
		m_mapClf.SetAt(nIndex, clf);
		m_SubeditEnable.SetAtGrow(nIndex,editEnable);
	}
	void SetSubItemText(int iSubItem, const CString& strSubItem){ m_SubItems.SetAtGrow(iSubItem, strSubItem);m_SubeditEnable.SetAtGrow(iSubItem,FALSE); }
	void SetSubItemText(int iSubItem, const CString& strSubItem, COLORREF clf)
	{
		...
		m_SubeditEnable.SetAtGrow(iSubItem,FALSE);
	}
	void SetSubItemText(int iSubItem, const CString& strSubItem, BOOL editEnable)
	{
		m_SubItems.SetAtGrow(iSubItem, strSubItem);
		m_SubeditEnable.SetAtGrow(iSubItem,editEnable);
	}
	void SetSubItemText(int iSubItem, const CString& strSubItem, COLORREF clf,BOOL editEnable)
	{
		m_SubItems.SetAtGrow(iSubItem, strSubItem);
		m_mapClf.SetAt(iSubItem, clf);
		m_SubeditEnable.SetAtGrow(iSubItem,editEnable);
	...
	INT_PTR GetItemCount(void) const { return m_SubItems.GetSize(); }

	enum CONTROLTYPE {edit/*default*/, combobox, datecontrol/*your control*/, spinbutton/*your control*/, dropdownlistviewwhatevercontrol,zftablelist/*your control*/};
	//all cols in this row is default edit

	////////值校验begin houmg
	typedef struct STRUCT_VALUE_CHECK{
		BOOL      bException;
		float     VMin;
		float     VMax;
		float     VException;
	...
	void SetSubItemValueCheck(int iSubItem,float fMin,float fMax,BOOL bException = FALSE)
	{
		stValueCheck tmp;
		tmp.bException = bException;
		tmp.VMin = fMin;
		tmp.VMax = fMax;
		tmp.VException = 255;
		m_valueCheck.SetAt(iSubItem,tmp);
	}

	BOOL GetSubItemValueCheck(int iSubItem,stValueCheck &stCheck)
	{
		if(m_valueCheck.Lookup(iSubItem,stCheck))
		{
			return TRUE;
		}
		else
	...
	void CopyObjects(CItemInfo* pItemInfo)
	{
		...
		memcpy(dumy,pItemInfo->dumy,sizeof(dumy));
	}
...
public:
	...
	BOOL GetEditEnable(int nIndex)
	{
		if((nIndex - 1) < m_SubeditEnable.GetSize())
		{
			if(nIndex == 0)
			{
				return m_editEnable;
			}
			return m_SubeditEnable.GetAt(nIndex - 1);
		}
		else return FALSE;
	}
	void SetEditEnable(int nIndex,BOOL editEnable)
	{
		if((nIndex - 1) < m_SubeditEnable.GetSize())
		{
			if(nIndex == 0)
			{
				m_editEnable = editEnable;
				return;
			}
			m_SubeditEnable.SetAtGrow(nIndex - 1,editEnable);
		}
	}
...
private:
	...
	CMap<int,int,stValueCheck,stValueCheck&> m_valueCheck;//houmg
	...
	///
	CArray<BOOL,BOOL> m_SubeditEnable;
	BOOL m_editEnable;
};
...
//
/////////////////////////////////////////////////////////////////////////////
class CSuperGridCtrl : public CMFCListCtrl
{
// Construction
public:
	CSuperGridCtrl();
...
public:
	...
	INT_PTR GetRootCount() { return m_RootItems.GetCount();}
