...
BEGIN_MESSAGE_MAP(CSuperGridCtrl, CMFCListCtrl)
		...
		{		
			...
			if(nIndex!=-1)
			{
				...
				if (pSelItem != NULL)
				{
					...
					else//如果不含有子项，并且为可编辑原件则显示编辑框
					{
						CItemInfo* lp;
						lp = GetData(pSelItem);
						
						if((m_CurSubItem != -1)&&(lp->GetEditEnable(m_CurSubItem)))
						{
							EditLabelEx(nIndex, m_CurSubItem );
						}	
					}
				}//!=NULL
			}
...
{
	/*
    if(m_bIsDragging)  //houmg 注释掉，防止拖拽
    {
				...
				case VK_LEFT:
					{
						...
						if(m_CurSubItem < 0) 
							...
							MakeColumnVisible( Header_OrderToIndex( GetHeaderCtrl().GetSafeHwnd(), m_CurSubItem));
							// Invalidate the item.
				...
				case VK_RETURN://edit itemdata
					{
						...
						if(!bResult)
						{
							...
							if( m_CurSubItem != -1 && iItem != -1)
							{
								...
								int iSubItem = Header_OrderToIndex(GetHeaderCtrl().GetSafeHwnd(), m_CurSubItem);
							/*	if(iSubItem==0)//that's just me saying all nodes in col 0 are edit-controls, you may modify this
								...
								if((lp->GetEditEnable(m_CurSubItem))&&(lp != NULL))
...
{
	//CHeaderCtrl* pHeader = CListCtrl::GetHeaderCtrl();
	return GetHeaderCtrl() ? GetHeaderCtrl().GetItemCount() : 0;
}
...
void CSuperGridCtrl::MakeColumnVisible(int nCol)
{
	...
	Header_GetOrderArray(GetHeaderCtrl().GetSafeHwnd(), nColCount, pOrderarray);
...
CSuperGridCtrl::CTreeItem* CSuperGridCtrl::InsertItem(CTreeItem *pParent, CItemInfo* lpInfo,  BOOL bUpdate)
{
	...
	else
	{
		...
		CMFCListCtrl::InsertItem(&lvItem);
		if(lpInfo->GetCheck())
			SetCheck(nIndex);
		//Get subitems
		int nSize = int(GetData(pItem)->GetItemCount());
	...
	if(ItemHasChildren(pSelItem) && IsCollapsed(pSelItem))
	{
		...
		while(pos != NULL)
		{
			...
			if(GetData(pNextNode)->GetCheck())
				SetCheck(nIndex + 1);
			//get subitems
			int nSize = int(GetData(pNextNode)->GetItemCount());
			for(int i=0; i< nSize;i++)
			{
...
void CSuperGridCtrl::OnUpdateListViewItem(CTreeItem* lpItem, LV_ITEM *plvItem)
{
	...
	if(lp!=NULL)
	{
		...
		{
			strold = lp->GetItemText();
			lp->SetItemText(str,lp->GetEditEnable(0));
			if(strold != str)
			{
				bFlag = TRUE;
			}
		}
		else //进行值校验，如果校验成功切发生改变，通知在＝更新数据 //houmg 
		{
			CItemInfo::stValueCheck  tmpCkeck;
			strold = lp->GetSubItem(plvItem->iSubItem - 1);
	
			if(lp->GetSubItemValueCheck(plvItem->iSubItem-1,tmpCkeck))
			{
				if(CString2Float(str,fTmp))
				{
					if((fTmp > tmpCkeck.VMax)||(fTmp < tmpCkeck.VMin))
					{
						if((tmpCkeck.bException) && (tmpCkeck.VException == fTmp))
						{
							bFlag = TRUE;
						}
						else
						{
							CString strTmp;
							strTmp.Format(_T("输入错误，请输入%4.0f 至 %4.0f的数字"),tmpCkeck.VMin,tmpCkeck.VMax);
				...
				{
					AfxMessageBox(_T("输入的值不是一个数字,请重新输入!"));
					bFlag = FALSE;
				}
			}
			else
		...
		if(bFlag)
		{
			lp->SetSubItemText(plvItem->iSubItem-1, str,lp->GetEditEnable(plvItem->iSubItem));
			SetItemText(plvItem->iItem, plvItem->iSubItem, str);
			GetParent()->SendMessage(WM_UPDATELIST,NULL,NULL);
		}
		else
		{
			lp->SetSubItemText(plvItem->iSubItem-1, strold,lp->GetEditEnable(plvItem->iSubItem));
			SetItemText(plvItem->iItem, plvItem->iSubItem, strold);
		}
...
{
	if((nIndex < 0)||(nIndex >=  this->GetCount()))   //houmg 20081017排异常
	{
		return NULL;
	}
...
int CSuperGridCtrl::getSelectedTreeItemType(void)
{
	CTreeItem * curTreeItem = GetTreeItem(GetSelectedItem());
	if (curTreeItem == NULL)
	{
		return -1;
	}

	if((curTreeItem->m_lpNodeInfo) == NULL)
	{
		return -1;
	}

	return	(curTreeItem->m_lpNodeInfo)->getItemType();
}
